...
const EventActionsMenu: React.FC<EventActionsMenuProps> = ({
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      // 删除成功后重定向到主页
      setAlert({ show: true, type: 'success', message: t('deleteEventSuccess') });
      router.push('/');
    } catch (error) {
      console.error('删除事件时发生错误:', error);
      setAlert({ show: true, type: 'error', message: error instanceof Error ? error.message : t('deleteEventError') });
    } finally {
      setIsDeleting(false);
    }
  };
