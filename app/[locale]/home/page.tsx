...
export default function MyProfilePage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {/* Tab Content - 使用条件渲染来确保组件只在首次加载时初始化 */}
      <div className="mt-6 min-h-[500px]">
        {/* 始终渲染已经缓存的tab，但只显示当前活跃的tab */}
        {tabCache.events && activeTab === 'events' && <CachedEventsTab />}
        {tabCache.timelines && activeTab === 'timelines' && <CachedTimelinesTab />}
        {tabCache.storyMaps && activeTab === 'storyMaps' && <CachedStoryMapsTab />}
        {tabCache.collections && activeTab === 'collections' && <CachedCollectionsTab />}
      </div>
    </div>
  );
}